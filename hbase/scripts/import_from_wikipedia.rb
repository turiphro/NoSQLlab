...
puts "(re)creating table 'wiki'"
conf = HBaseConfiguration.new
admin = HBaseAdmin.new( conf )
# some optimisation
table = drop_and_create( admin, 'wiki', ['text', 'revision'] )
#table = HTable.new( admin.getConfiguration(), 'wiki' )
disable 'wiki'
...
table.setAutoFlush( false ) # (2)  buffer until .flushCommits()
